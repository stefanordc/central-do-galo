...
from app.services.jogador_service import (
    atualizar_jogador,
    criar_jogador,
    excluir_jogador,
    listar_elenco_admin,
...
class JogadorElencoIn(BaseModel):
    nome_completo: str = Field(min_length=2, max_length=220)
    nome_guerra: str = Field(min_length=1, max_length=120)
    posicao: Literal[
        "Goleiro",
        "Lateral-Direito",
        "Lateral-Esquerdo",
    ...
    nacionalidade: str = Field(default="Brasil", min_length=2, max_length=100)
    nascimento: date
    numero: int | None = Field(default=None, ge=0, le=999)
...
@router.get("/elenco")
def get_elenco_admin(authorization: str | None = Header(default=None)) -> list[dict]:
    _validar_admin(authorization)
    return listar_elenco_admin()


@router.post("/elenco", status_code=status.HTTP_201_CREATED)
def post_jogador_elenco_admin(
    payload: JogadorElencoIn,
    authorization: str | None = Header(default=None),
) -> dict:
    _validar_admin(authorization)
    try:
        return criar_jogador(
        ...
        raise HTTPException(
            status_code=409,
            detail="Esse número de camisa já está em uso no elenco atual.",
        ) from exc


@router.put("/elenco/{jogador_id}")
def put_jogador_elenco_admin(
    jogador_id: UUID,
    payload: JogadorElencoIn,
    authorization: str | None = Header(default=None),
) -> dict:
    ...
    try:
        return atualizar_jogador(
            ...
            nacionalidade=payload.nacionalidade,
            foto_data_url=payload.foto_data_url,
            remover_foto=payload.remover_foto,
        )
...
@router.delete("/elenco/{jogador_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_jogador_elenco_admin(
    jogador_id: UUID,
    authorization: str | None = Header(default=None),
) -> None:
    _validar_admin(authorization)
    try:
        excluir_jogador(jogador_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
