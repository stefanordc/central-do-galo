...
_REMOTE_URL = (os.getenv("X_REMOTE_STORE_URL") or "").strip()
_REMOTE_TOKEN = (os.getenv("X_REMOTE_STORE_TOKEN") or "").strip()
_OIDC_AUDIENCE = "central-do-galo-x"
...
def _github_oidc_token() -> str:
    ...
    if not request_url or not request_token:
        raise RuntimeError(
            "X_REMOTE_STORE_URL está definido, mas o token OIDC do GitHub Actions não está disponível."
        )
...
def remote_call(action: str, **payload):
    if not _REMOTE_URL:
        raise RuntimeError("X_REMOTE_STORE_URL não configurada.")
