...
def sincronizar_agenda() -> dict[str, Any]:
    ...
    passados: list[dict[str, Any]] = []
    futuros: list[dict[str, Any]] = []
    ...
    for item in fixtures:
        ...
        try:
            inicio_jogo = datetime.fromisoformat(
                str(data_texto).replace("Z", "+00:00")
            )
        except ValueError:
            futuros.append(item)
            continue
        ...
        if inicio_jogo.tzinfo is None:
            inicio_jogo = inicio_jogo.replace(tzinfo=UTC)

        if inicio_jogo <= agora:
    ...
    return {
        ...
        "encontrados": len(fixtures),
        "passados": len(passados),
        "futuros": len(futuros),
        "salvos": salvos,
...
def _temporadas_para_testar() -> list[int]:
    ano_atual = date.today().year
    return list(range(ano_atual, 1999, -1))
...
def sincronizar_historico_maximo() -> dict[str, Any]:
    ...
    temporadas = _temporadas_para_testar()

    resultados: list[dict[str, Any]] = []
    total_salvos = 0
    temporadas_com_dados = 0

    for temporada in temporadas:
        if ULTIMA_COTA_RESTANTE is not None and ULTIMA_COTA_RESTANTE <= 8:
            logger.warning(
        ...
        try:
            ...
            logger.warning(
                "[Jogos] temporada %s indisponível no plano atual: %s",
                temporada,
                exc,
            )
            resultados.append(
    ...
    return {
        "team_id": team_id,
        "team_name": team.get("name"),
        "temporadas_testadas": temporadas,
        "temporadas_com_dados": temporadas_com_dados,
...
def sincronizacao_inicial() -> dict[str, Any]:
    ...
    try:
        ...
        logger.exception("[Jogos] falha na importação histórica")
        erros.append(f"historico: {exc}")

    if ULTIMA_COTA_RESTANTE is None or ULTIMA_COTA_RESTANTE > 5:
        try:
            _esperar_cota()
            agenda = sincronizar_agenda()
        except Exception as exc:
            logger.exception("[Jogos] falha na sincronização da agenda")
            erros.append(f"agenda: {exc}")

    if ULTIMA_COTA_RESTANTE is None or ULTIMA_COTA_RESTANTE > 10:
        try:
            _esperar_cota()
            gols = sincronizar_autores_gols(limite=12)
        except Exception as exc:
            logger.exception("[Jogos] falha ao sincronizar autores dos gols")
