...
type SecaoAtiva = "noticias" | "videos" | "x" | "jogos" | "dados" | "elenco";

export default function CentralDoGaloPage({
  initialSection = "noticias",
}: {
  ...
  const [abaDados, setAbaDados] = useState<"coletivos" | "individual">("coletivos");
  const [anosDados, setAnosDados] = useState<number[]>([]);
  const [anoDados, setAnoDados] = useState(new Date().getFullYear());
  const [statusDados, setStatusDados] = useState<DadosStatus | null>(null);
  const [dadosColetivos, setDadosColetivos] = useState<DadosColetivos | null>(null);
  const [dadosIndividuais, setDadosIndividuais] = useState<DadosIndividuais | null>(null);
  ...
  function navegarPara(secao: SecaoAtiva) {
    const rotas: Record<SecaoAtiva, string> = {
      noticias: "/noticias",
      videos: "/videos",
      x: "/x",
      jogos: "/jogos",
  ...
  async function carregarNoticias({ append = false }: { append?: boolean } = {}) {
    ...
    try {
      ...
      const dataFiltrada = data.filter((noticia) => {
        if (noticia.fonte_slug !== "noataque-atletico") return true;
        const titulo = normalizarBusca(noticia.titulo);
        return !["atletico de madrid", "atletico madrid", "laliga"].some((termo) =>
          titulo.includes(termo)
        );
      });
      setNoticias((atuais) => (append ? [...atuais, ...dataFiltrada] : dataFiltrada));
  ...
  async function carregarElenco() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`API do elenco respondeu ${response.status}`);
      }

      const data: JogadorElenco[] = await response.json();
      setElenco(data);
      setErroElenco(null);
    } catch (error) {
      setErroElenco(
        error instanceof Error
          ? error.message
          : "Não foi possível carregar o elenco atual."
      );
    } finally {
  ...
  async function carregarDados(ano: number, aba: "coletivos" | "individual") {
    setCarregandoDados(true);
    try {
      const [anosResponse, statusResponse, dadosResponse] = await Promise.all([
        fetch(`${API_URL}/api/dados/anos`, { cache: "no-store" }),
        fetch(`${API_URL}/api/dados/status?ano=${ano}`, { cache: "no-store" }),
        fetch(`${API_URL}/api/dados/${aba}?ano=${ano}`, { cache: "no-store" }),
      ]);

      if (anosResponse.ok) {
        const body = await anosResponse.json();
        const anos: number[] = body.anos ?? [];
        setAnosDados(anos);
      ...
      if (!statusResponse.ok) {
        throw new Error(`API de dados respondeu ${statusResponse.status}`);
      }
      if (!dadosResponse.ok) {
        throw new Error(`API de dados respondeu ${dadosResponse.status}`);
      }

      setStatusDados(await statusResponse.json());
      if (aba === "coletivos") {
        setDadosColetivos(await dadosResponse.json());
      } else {
        setDadosIndividuais(await dadosResponse.json());
      }
      setErroDados(null);
    } catch (error) {
      setErroDados(
    ...
    <main className="page-shell">
        ...
        <div className="hero-copy">
          <div className="brand-lockup">
            ...
            <div>
              ...
              <p className="brand-tagline">Notícias · Vídeos · X · Jogos · Dados · Elenco</p>
            </div>
          </div>
      ...
      <nav className="menu">
        <button
          ...
          onClick={() => navegarPara("noticias")}
        >
          Notícias
        </button>
        <button
          className={secaoAtiva === "videos" ? "active" : ""}
          onClick={() => navegarPara("videos")}
        >
          Vídeos
        </button>
        <button
          className={secaoAtiva === "x" ? "active" : ""}
          onClick={() => navegarPara("x")}
        >
          X
        </button>
        <button
        ...
        <button
          ...
          onClick={() => navegarPara("dados")}
        >
          Dados
        </button>
        <button
          className={secaoAtiva === "elenco" ? "active" : ""}
          onClick={() => navegarPara("elenco")}
        >
          Elenco
        </button>
      </nav>
          ...
          <section className="games-calendar-shell">
            ...
            {!erroJogos && !carregandoJogos && statusJogos?.total === 0 && (
              <div className="state-card games-state">
                ...
                <span>
                  Execute a sincronização inicial do Sofascore no backend.
                </span>
              </div>
            )}
      ...
      {secaoAtiva === "dados" && <DadosSection />}

      {secaoAtiva === "elenco" && (
        <>
          <section className="section-header squad-section-header">
            <div>
              <p className="eyebrow">ELENCO ATUAL</p>
              <h2>Elenco</h2>
            </div>
          </section>
          ...
          {erroElenco && (
            <div className="state-card error-card">
              ...
              <span>{erroElenco}</span>
            </div>
          )}

          {!erroElenco && carregandoElenco && elenco.length === 0 && (
            <div className="state-card">
              <strong>Carregando elenco...</strong>
            </div>
          )}

          {!erroElenco && !carregandoElenco && elenco.length === 0 && (
            <div className="state-card">
              <strong>Nenhum jogador cadastrado ainda.</strong>
              <span>Use o painel administrativo para montar o elenco atual.</span>
            </div>
          )}

          {!erroElenco && elenco.length > 0 && (
            <section className="squad-grid" aria-label="Elenco atual do Atlético">
              {elenco.map((jogador) => (
                <article className="squad-card" key={jogador.id}>
                  <div className="squad-photo-wrap">
                    {jogador.foto_url ? (
                      <img
                        className="squad-photo"
                        src={jogador.foto_url}
                        alt={jogador.nome_guerra}
                        loading="lazy"
                        decoding="async"
                      />
                    ) : (
                      <div className="squad-photo-fallback">
                        CAM
                      </div>
                    )}

                    {jogador.numero != null && (
                      <span className="squad-number">{jogador.numero}</span>
                    )}
                  </div>

                  <div className="squad-card-body">
                    <h3>{jogador.nome_guerra}</h3>
                    <span className="squad-position">{jogador.posicao}</span>
                    <p>{jogador.nome_completo}</p>

                    <div className="squad-meta">
                      <div>
                        <span>Idade</span>
                        <strong>{jogador.idade} anos</strong>
                      </div>
                      <div>
                        <span>Nascimento</span>
                        <strong>{jogador.nascimento.split("-").reverse().join("/")}</strong>
                      </div>
                    </div>
                  </div>
                </article>
      ...
      <style jsx global>{`
        ...
        .data-player-cell div {
          display: grid;
          gap: 2px;
        }

        .data-player-cell span {
          color: #868c97;
          font-size: 11px;
        }
